...
function marvel_movie_selector_css() {
	...
	.rtl #marvel_movie_selector {
		float: left;
	}
	.block-editor-page #marvel_movie_selector {
		display: none;
	}
...
function watch_marvel_movie_selector() {
	...
	$chosen = $selected_marvel_movie_selector;
	$chosen = "Marvel MCU movie to watch is: " . $chosen;
	...
	printf(
		'<p id="marvel_movie_selector"><span class="screen-reader-text">%s </span><span dir="ltr"%s>%s</span></p>',
		__( '', '' ),
		$lang,
		$chosen
	);
...
//     //$uses = $item[$obj[0]['id']]; //etc
// }
//debug_to_console($uses);
